...
public class MoviesApiClientGrpc : IMoviesApiClient
{
    ...
    public async Task<showListResponse> GetAllAsync()
    {
        ...
        {
            throw new MovieApiUnAvailableException(
                $"process for fetching all movie data failed. Error message => {ex.Message}");
        }
    }
    ...
    public async Task<showResponse?> GetByIdAsync(string movieId, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new MovieApiUnAvailableException(
                $"process for fetching movie data failed. Error message => {ex.Message}");
        }
    }
